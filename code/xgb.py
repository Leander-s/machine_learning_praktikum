...
def hyper_runing(subtask):
    ...
    def hyper_opt(args):
        ...
        model = XGBClassifier(**args, n_jobs=6, random_state=1, seed=1,
                              scale_pos_weight=pos_weight) if task_type == 'cla' else XGBRegressor(**args, n_jobs=6,
                                                                                                   random_state=1,
                                                                                                   seed=1)
    ...
    best_model = XGBClassifier(n_estimators=n_estimators_ls[best_results['n_estimators']],
                               ...
                               n_jobs=6, random_state=1, seed=1, scale_pos_weight=pos_weight) \
...
if task_type == 'cla':
    best_hyper = pd.DataFrame(pd_res, columns=['dataset', 'subtask', 'set',
                                               ...
                                               'learning_rate', 'gamma', 'subsample', 'colsample_bytree',
                                               'tn', 'fp', 'fn', 'tp', 'se', 'sp',
    ...
    best_hyper = pd.DataFrame(pd_res, columns=['dataset', 'subtask', 'set',
                                               ...
                                               'learning_rate', 'gamma', 'subsample', 'colsample_bytree',
                                               'rmse', 'r2', 'mae'])
best_hyper.to_csv('./stat_res/' + dataset_label + '_moe_pubsub_xgb_hyperopt_info.csv', index=0)
...
stat_res.to_csv('./stat_res/' + dataset_label + '_xgb_statistical_results_split50_20200622.csv', index=0)
# single tasks
if len(tasks) == 1:
...
# multi-tasks
    ...
    args = {'data_label': dataset_label, 'metric': 'auc_roc' if dataset_label != 'muv' else 'auc_prc', 'model': 'XGB'}
